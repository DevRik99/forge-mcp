...
## Gates que aplican (este repo)

- `npm run lint` — complexity 10, max-lines-per-function 40, max-depth 3, max-params 4, no-magic-numbers (warn; `check-eslint-warnings-ratchet.mjs` impide que crezcan)
- `npm run dup:check` — duplicación estructural (ratchet)
- `npm run dead:check` — código muerto con knip (ratchet)
- `npm run nombres:check` y `npm run filenames:check` — identificadores y nombres de archivo en inglés (ratchet); cspell sobre módulos migrados

Fuente única: `npm run verify`.
